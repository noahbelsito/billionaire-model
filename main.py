...
billionaire_model.fit(X, y)
predictions = billionaire_model.predict(X)
...
val_predictions = billionaire_model.predict(val_X)  # validate current model using validation data
mae = mean_absolute_error(val_y, val_predictions)  # mae using val_y and val_predictions
